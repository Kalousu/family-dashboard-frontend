...
import { SIDEBAR_VIEWS, type SideBarViewType } from "../../../types/sidebarTypes"
...
function SideBar({ isOpen, onClose, pendingWidget, setPendingWidget, permissions }: SideBarProps) {
    const [sideBarView, setSideBarView] = useState<SideBarViewType>(SIDEBAR_VIEWS.NAV)
    ...
    return (
        ...
        <div className={`fixed z-20 right-0 top-0 h-full w-70 rounded-l-2xl p-4 transition-all duration-300 will-change-transform flex flex-col justify-between backdrop-blur-md ${isOpen ? "translate-x-0" : "translate-x-full"} ${isDarkMode ? "bg-linear-to-b from-gray-950/90 via-gray-900/85 to-slate-900/80 border-2 border-white/5" : "bg-linear-to-b to-sky-100 from-blue-200 border-2 border-gray-400/20"}`}>
            {sideBarView === SIDEBAR_VIEWS.NAV ? (
                    ...
                    <div>
                        ...
                        <div className="m-3 flex flex-col items-center">
                            <div className={`relative rounded-2xl p-2 bg-linear-to-b transition-all hover:scale-103 hover:brightness-105 ${isDarkMode ? "from-gray-500/50 via-gray-600/20 to-blue-400/20 border-2 border-white/10" : "from-sky-200/30 via-slate-400/15 to-blue-400/20 border-2 border-cyan-950/5"}`}>
                                ...
                                {currentUser && <AvatarDisplay user={currentUser} size="lg" />}
                            </div>
                        ...
                        <SideBarNav onWidgetsClick={() => setSideBarView(SIDEBAR_VIEWS.WIDGETS)} onAdminClick={() => setSideBarView(SIDEBAR_VIEWS.ADMIN)} permissions={permissions}/>
                    </div>
            ...
            ) : sideBarView === SIDEBAR_VIEWS.ADMIN ? (
                <div className="h-full overflow-y-auto">
                    <AdminDrawer onBack={() => setSideBarView(SIDEBAR_VIEWS.NAV)} isDarkMode={isDarkMode} />
                </div>
            ) : (
                <div className="h-full overflow-y-auto">
                    <WidgetDrawer onBack={() => setSideBarView(SIDEBAR_VIEWS.NAV)} pendingWidget={pendingWidget} setPendingWidget={setPendingWidget} />
                </div>
            )}
        </div>
        </>
