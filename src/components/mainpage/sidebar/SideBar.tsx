    ...
    return(
        <div className={`fixed z-20 right-0 top-0 h-full w-70 bg-linear-to-b from-gray-700 to-gray-600 rounded-l-2xl p-4 border-2 border-white/5 transition-transform duration-300 flex flex-col justify-between ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
            <div>
                ...
                <SideBarNav isDarkMode={isDarkMode} onToggleDarkMode={onToggleDarkMode}/>
            </div>
            <button className="self-center mb-4 p-3 w-30 bg-linear-to-b from-gray-500/70 to-gray-600 text-gray-200 font-semibold rounded-xl border-2 border-white/10 hover:scale-102 hover:brightness-110 transition-all ease-in-out duration-200">Abmelden</button>
        </div>
    )
}
