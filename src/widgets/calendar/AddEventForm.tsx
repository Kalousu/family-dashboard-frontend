...
import type { FormProps } from "./calendarTypes";

type Props = FormProps & { isDarkMode: boolean };

function FormHeader({ isDarkMode, editingEventId, onCancel, onSubmit }: {
    isDarkMode: boolean;
...
function AllDayToggle({ formAllDay, onAllDayToggle }: { formAllDay: boolean; onAllDayToggle: () => void }) {
    return (
...
function TimePicker({ formTime, onTimeChange }: { formTime: string; onTimeChange: (t: string) => void }) {
...
function ColorSelector({ formColor, onColorChange, showColorPicker, onPickerButtonClick, onColorPickerClose, localPickerColor, onPickerColorChange }: {
    formColor: string;
            ...
            <AnimatePresence>
                {showColorPicker && (
                    ...
                    >
                        ...
                        >
                            <HslStringColorPicker color={localPickerColor} onChange={onPickerColorChange} />
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
...
function AddEventForm({
    ...
    return createPortal(
        ...
        >
            ...
            >
                <FormHeader isDarkMode={isDarkMode} editingEventId={editingEventId} onCancel={onCancel} onSubmit={onSubmit} />
                ...
                <AllDayToggle formAllDay={formAllDay} onAllDayToggle={onAllDayToggle} />
                ...
                <AnimatePresence initial={false} mode="popLayout">
                    {!formAllDay && (
                        ...
                        >
                            <TimePicker formTime={formTime} onTimeChange={onTimeChange} />
                        </motion.div>
                    )}
                </AnimatePresence>

                <ColorSelector
                    formColor={formColor} onColorChange={onColorChange}
                    showColorPicker={showColorPicker} onPickerButtonClick={onPickerButtonClick}
                    onColorPickerClose={onColorPickerClose}
                    localPickerColor={localPickerColor} onPickerColorChange={onPickerColorChange}
                />
            </motion.div>
        </div>,
        document.body
